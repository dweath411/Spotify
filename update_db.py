...
def update_playlist_history_schema():
    """
    Update the playlist_history table schema to include the 'timestamp' column.
    """
    conn = sqlite3.connect("app.db")
    cursor = conn.cursor()
    ...
    # Copy data from the old table
    cursor.execute("""
        INSERT INTO playlist_history (user_id, playlist_name, playlist_id, song_uris)
        SELECT user_id, playlist_name, playlist_id, song_uris FROM playlist_history_old
    """)
    ...
    conn.commit()
    conn.close()
    print("Updated playlist_history table schema successfully!")
